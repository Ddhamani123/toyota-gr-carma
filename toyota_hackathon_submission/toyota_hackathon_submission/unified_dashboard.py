...
# ------------------------
# Simulation helpers (shared with COTA/Barber fastest-lap playback)
# ------------------------
SIM_CONFIG = {
    "COTA": {
        "fastest_laps": Path("fastest_laps.parquet"),
        "fastest_telemetry": Path("fastest_lap_telemetry.parquet"),
        "layout": None,
    },
    "Barber": {
        "fastest_laps": Path("barber_fastest_laps.parquet"),
        "fastest_telemetry": Path("barber_fastest_lap_telemetry.parquet"),
        "layout": Path("scripts/barber_track_layout.csv"),
    },
}

def cota_polyline() -> np.ndarray:
    """Approximate COTA centerline, using the mapped coordinates from earlier sim."""
    ref_points = [
        (98, 466, 115, 460),
        (290, 660, 303, 644),
        (303, 644, 318, 662),
...
def barber_polyline(layout_path: Optional[Path] = None):
...
def build_sim_frames(fastest_df: pd.DataFrame, tele_df: pd.DataFrame, car_numbers, polyline: np.ndarray):
    if tele_df.empty or fastest_df.empty:
        return pd.DataFrame()
    frames = []
    ...
    for car in car_numbers:
        ...
        if row.empty:
            continue
        row = row.iloc[0]
        lap_time = float(row["LAP_TIME_SEC"])
...
def simulation_plot(sim_df: pd.DataFrame, polyline: np.ndarray, sector_fracs: Tuple[float, float]):
    if sim_df.empty:
        return go.Figure()
    track = polyline.copy()
    # clamp sector fractions
    ...
    # if any slice is too small to be visible, fall back to thirds just for drawing
    ...
    # ensure each sector has at least two points by using index splits
    n = len(progress)
    idx1 = max(1, int(round(s1_frac * (n - 1))))
    idx2 = max(idx1 + 1, int(round(s2_frac * (n - 1))))
    segs = []
    segs.append(go.Scatter(x=tx[: idx1 + 1], y=ty[: idx1 + 1], mode="lines", line=dict(color="#3b82f6", width=3), name="S1"))
    segs.append(go.Scatter(x=tx[idx1: idx2 + 1], y=ty[idx1: idx2 + 1], mode="lines", line=dict(color="#f59e0b", width=3), name="S2"))
    segs.append(go.Scatter(x=tx[idx2:], y=ty[idx2:], mode="lines", line=dict(color="#10b981", width=3), name="S3"))
    fig = go.Figure()
    for tr in segs:
        fig.add_trace(tr)
    ...
    fig.update_layout(
        ...
        legend=dict(x=0.8, y=1, bgcolor="rgba(255,255,255,0.2)"),
        updatemenus=[
            {
                "type": "buttons",
                "buttons": [
...
def main():
            ...
            if st.button("Load & Analyze Telemetry", type="primary"):
                with st.spinner(f"Aligning telemetry for Driver {ref_driver} vs {target_driver}..."):
                    ...
                    if ref_raw.empty or target_raw.empty:
                        ...
                        # Clean NaNs before projection to avoid KDTree errors
                        ref_clean = st.session_state['ref_telemetry'].dropna(subset=['x', 'y'])
                        target_clean = target_processed.dropna(subset=['x', 'y'])
                        # 1. Generate Master Centerline from Reference Lap
                        # We use the reference lap as the "Centerline" definition
                        centerline_data = track_projection.generate_centerline(ref_clean)
                        
                        # 2. Project Reference to Centerline (Self-projection to get Station)
                        ref_proj = track_projection.project_to_centerline(ref_clean, centerline_data)
                        
                        # 3. Project Target to Centerline
                        target_proj = track_projection.project_to_centerline(target_clean, centerline_data)
    ...
    # ---------------- Simulation (Fastest Lap) ----------------
    if track_name in SIM_CONFIG:
        ...
        if fastest_sim.empty or tele_sim.empty or track_poly is None:
            ...
            cars = list(dict.fromkeys([target_driver, ref_driver])) if analysis_mode == "Head-to-Head" else [target_driver, ref_driver]
            cars = [c for c in cars if not pd.isna(c)]
            base_row = fastest_sim[fastest_sim["CAR_NUMBER"].isin(cars)].head(1)
            if base_row.empty:
                st.info("No fastest-lap entries found for selected drivers.")
            else:
                s1_frac, s2_frac = 0.33, 0.66
                if set(["S1_SEC", "S2_SEC", "S3_SEC"]).issubset(fastest_sim.columns):
                    # pick a car with finite sector times if available
